...
class Conta(QDialog):
    ...
    def ui(self):
        ...
        grid.addRow(QLabel("Endereço do Agência"), self.endereco_banco)
        ...
        self.setWindowTitle("Cadastro de Contas")
    ...
    def validacao(self):
        ...
        if self.nome.text() == "":
            ...
            self.nome.setFocus()
            return False
        elif self.conta.text() == "":
            QMessageBox.information(self, "Erro", "Número de Conta Inválido")
            self.conta.setFocus()
            return False
        else:
            return True
    ...
    def mostrar_registo(self, codigo):
        ...
        sql = """SELECT * from contas WHERE cod={} """.format(codigo)
    ...
    def existe(self, codigo):
        ...
        sql = """SELECT cod from contas WHERE cod = "{codigo}" """.format(codigo=str(codigo))

        self.cur.execute(sql)
        data = self.cur.fetchall()

        if len(data) == 0:
            self.cod_conta = codigo
            return False
        else:
            codigo = int(data[0][0])
            self.cod_conta = codigo
    ...
    def add_record(self):
        ...
        if self.existe(cod) is True:
            ...
            sql = """UPDATE contas set nome_banco="{nome_banco}", conta="{conta}", nib="{nib}", swift="{swift}",
            ...
            sql = """INSERT INTO contas (nome_banco, conta, nib, swift, endereco, estado, obs, 
