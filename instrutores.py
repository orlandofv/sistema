...
class Instrutor(QDialog):

    def __init__(self, parent=None):
        super(Instrutor, self).__init__(parent)
    ...
    def ui(self):
        html = """<center style= "background-image: './images/control.png'" > <h2 > Cadastro de Instrutores </h2> </center> """
        ...
        self.setWindowTitle("Cadastro de Instrutores")
    ...
    def limpar(self):
        ...
        # gera novo codigo para instrutores
        from utilities import codigo
    ...
    def validacao(self):
        ...
        if str(self.nome.text()) == "":
            QMessageBox.warning(self, "Erro", "Nome do Instrutor inválido")
            self.nome.setFocus()
            return False
        else:
            return True
    ...
    def mostrar_registo(self):
        ...
        sql = """SELECT * FROM instrutores where cod="{}" """.format(self.current_id)
    ...
    def existe(self, codigo):
        ...
        sql = """SELECT cod from instrutores WHERE cod = "{codigo}" """.format(codigo=str(codigo))
    ...
    def addRecord(self):
        ...
        if self.validacao() == True:
            ...
            if self.current_id == "":
                self.current_id = "IS" + codigo("IS" + "abcdefghijklmnopqrstuvxwzABCDEFGHIJKLMNOPQRSTUVXWZ0123456789")

            code = self.current_id

            if self.existe(code):

                sql = """UPDATE instrutores SET nome="{nome}", apelido="{apelido}", endereco="{endereco}", sexo="{sexo}",
                                ...
                                created_by="{created_by}", foto="{foto}" WHERE cod="{cod}" """.format(
                ...
                values = """ "{cod}", "{nome}", "{apelido}", "{endereco}", "{sexo}", "{email}", "{nascimento}", 
                                ...
                                "{modified_by}", "{created_by}", "{foto}" """.format(cod=code, nome=nome,
                                                                                                  apelido=apelido,
                                                                                                  endereco=endereco,
                                                                                                  sexo=sexo,
                                                                                                  email=email,
                ...
                sql = """INSERT INTO instrutores VALUES ({}) """.format(values)
