...
class ListaDeEstudantes(QMainWindow):
    ...
    def ui(self):
        ...
        self.tv.setContextMenuPolicy(Qt.DefaultContextMenu | Qt.ActionsContextMenu)
    ...
    def fill_table(self):
        ...
        header = [qApp.tr('Código'), qApp.tr('Nome'), qApp.tr('Apelido'), qApp.tr('Empresa'),  qApp.tr('Nacionalidade'),
                  qApp.tr('Email'), "Contactos", "Documento", "Comentários"]

        self.sql = """select estudantes.cod, estudantes.nome, estudantes.apelido, clientes.nome, estudantes.nacionalidade, 
        estudantes.email, estudantes.contactos, estudantes.numero_id, estudantes.obs  
        from estudantes JOIN clientes on clientes.cod=estudantes.empresa 
        WHERE estudantes.nome like "%{nome}%" OR clientes.nome like "%{nome}%" """.format(nome=self.find_w.text())
    ...
    def removerow(self):
        ...
        if self.current_id != "CL20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                ...
                sql = """delete from estudantes WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))
                ...
                try:
                    ...
                    QMessageBox.warning(self, "Erro", "Impossivel apagar estudante.")
        else:
            QMessageBox.warning(self, "Erro", "Estudante não pode ser apagado.")
    ...
    def imprime_lista(self):
        ...
        for cliente in dados:
            ...
            html += """<tr> <td>{}</td><td>{}</td><td>{}</td><td>{}</td> <td>{}</td> <td>{}</td> <td>{}</td> <td>{}</td>  
...
#
#     helloPythonWidget = ListaDeEstudantes()
