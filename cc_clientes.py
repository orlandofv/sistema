...
class CC_Cliente(QDialog):
    cod_cc_cliente = 0
    cod_documento = 0

    def __init__(self, parent=None):
        super(CC_Cliente, self).__init__(parent)
    ...
    def ui(self):
        html = """<center style= "background-image: './images/control.png'" > <h2 > Conta Corrente de Clientes </h2> </center> """
        ...
        # grid.addRow(QLabel("Valor pago pelo Cliente"), self.valor_pago)
        # grid.addRow(QLabel("Saldo do Cliente"), self.saldo)
        ...
        self.setWindowTitle("Cadastro de Conta Corrente de Clientes")
    ...
    def mostrar_registo(self, codigo):
        ...
        sql = """SELECT * FROM cc_clientes WHERE cod={} """.format(codigo)
        self.cur.execute(sql)
        data = self.cur.fetchall()

        if len(data) > 0:
            for x in data:
                self.cliente.setCurrentText(x[1])
    ...
    def existe(self, codigo):
        ...
        sql = """SELECT cod from cc_clientes WHERE cod = "{codigo}" """.format(codigo=str(codigo))

        self.cur.execute(sql)
        data = self.cur.fetchall()

        if len(data) == 0:
            self.cod_cc_cliente = codigo
            return False
        else:
            codigo = int(data[0][0])
            self.cod_cc_cliente = codigo
    ...
    def add_record(self):
        ...
        if self.existe(cod) is True:
            ...
            sql = """UPDATE cc_clientes set cliente="{cliente}", documento="{documento}", data="{data}", iva={iva},
                        ...
                        """.format(cod=cod, cliente=cliente, documento=documento, data=data, iva=iva,
                                   valor_a_pagar=valor_a_pagar, valor_pago=valor_pago, saldo=saldo,
                                   estado=estado, modified=modified, modified_by=modified_by,
                                   obs=obs)

        else:
            values = """ "{cliente}", "{documento}", "{data}", "{iva}", {valor_a_pagar}, {valor_pago}, {saldo}, 
                                                {estado}, "{obs}", "{created}", "{modified}", "{modified_by}", 
                                                "{created_by}" 
                                                            """.format(cliente=cliente, documento=documento, data=data,
                                                                       iva=iva,
            ...
            sql = """INSERT INTO cc_clientes (cliente, documento, data, iva, valor_a_pagar, valor_pago, saldo, estado, 
