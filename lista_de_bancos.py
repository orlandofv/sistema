...
class Lista_de_Bancos(QMainWindow):
    def __init__(self, parent=None):
        super(Lista_de_Bancos, self).__init__(parent)
    ...
    def fill_table(self):
        ...
        header = [qApp.tr('Cod'), qApp.tr('Banco'), "Conta", qApp.tr('NIB'), qApp.tr('SWIFT'), qApp.tr('Endereço'),
                  qApp.tr('Notas')]

        self.sql = """select cod, nome_banco, conta, nib, swift, endereco, obs 
        FROM bancos WHERE  bancos.nome_banco like "%{nome}%" 
        OR bancos.conta like "%{nome}%" or bancos.nib like "%{nome}%" or bancos.swift like "%{nome}%"
        or bancos.endereco like "%{nome}%" 
    ...
    def removerow(self):
        ...
        if self.current_id != "CL20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                ...
                sql = """delete from bancos WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))
                ...
                try:
                    ...
                    QMessageBox.warning(self, "Erro", "Impossivel apagar banco.")
        else:
            QMessageBox.warning(self, "Erro", "Banco não pode ser apagado.")
    ...
    def imprime_lista(self):
        ...
        html = "<center> <h2> Lista de Bancos </h2> </center>"
        html += "<hr/>"
...
#
    helloPythonWidget = Lista_de_Bancos()
