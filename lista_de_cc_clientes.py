...
class Lista_de_CC_Cliente(QMainWindow):
    def __init__(self, parent=None):
        super(Lista_de_CC_Cliente, self).__init__(parent)

        # controla o codigo
        self.current_id = ""
        self.lista_de_cc_clientes = []
        self.list_de_dados_normal = []

        # Header for the table
        self.header = [qApp.tr('Cod'), qApp.tr('Cliente'), "Documento", qApp.tr('Data'), qApp.tr('IVA'),
    ...
    def fill_table(self):
        ...
        sql = """select cod, cliente, documento, data, iva, valor_a_pagar, valor_pago, saldo, 
            obs FROM cc_clientes WHERE  ((cliente like "%{nome}%" 
        ...
        try:
            ...
            print(data)
            self.lista_de_cc_clientes = data
    ...
    def new_data(self):
        ...
        from cc_clientes import CC_Cliente
        cl = CC_Cliente(self)
        cl.setModal(True)
        cl.show()
    ...
    def update_data(self):
        ...
        from cc_clientes import CC_Cliente
        cl = CC_Cliente(self)
        cl.cod_cc_cliente = self.current_id
    ...
    def removerow(self):
        ...
        if self.current_id != "CL20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                ...
                sql = """delete from cc_clientes WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))
            ...
            QMessageBox.warning(self, "Erro", "CC_Cliente não pode ser apagado.")

    def imprime_lista(self):
        """Imprime no ecra a Lista geral de produtos baseado em um Critério"""
        if self.lista_de_cc_clientes == []:
            return

        dados = self.lista_de_cc_clientes
...
#
    helloPythonWidget = Lista_de_CC_Cliente()
