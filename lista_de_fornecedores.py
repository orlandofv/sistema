...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.header = [qApp.tr('Código'), qApp.tr('Nome'), qApp.tr('Endereço'), qApp.tr('NUIT'), qApp.tr('Email'),
                       "Contactos", "Observações"]
    ...
    def fill_table(self):
        ...
        self.sql = """select cod, nome, endereco, NUIT, email, contactos, obs 
        from fornecedores WHERE nome like "%{}%" or contactos like "%{}%" 
        or endereco like "%{}%" """.format(self.find_w.text(), self.find_w.text(), self.find_w.text())
    ...
    def removerow(self):
        ...
        if self.current_id != "FR20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                                    QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
                sql = """delete from fornecedores WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))
                self.cur.execute(sql)
                self.conn.commit()
                self.fill_table()
                QMessageBox.information(self, "Sucesso", "Item apagado com sucesso...")
        else:
            QMessageBox.warning(self, "Erro", "Fornecedor não pode ser apagado.")
