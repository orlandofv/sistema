...
class Pagamento_de_Cliente(QDialog):
    cod_pagamento_de_cliente = 0
    cod_documento = 0
    cod_cc_cliente = 0
    ...
    def __init__(self, parent=None):
        super(Pagamento_de_Cliente, self).__init__(parent)
    ...
    def ui(self):
        html = """<center style= "background-image: './images/control.png'" > <h2 > Pagamento de Clientes </h2> </center> """

        titulo = QLabel(html)

        self.cliente = QComboBox()
        self.cliente.currentIndexChanged.connect(lambda: self.enche_documentos(self.cliente.currentText()))
        ...
        self.setWindowTitle("Cadastro de Pagamento de Clientes")
    ...
    def get_codcc(self, nome_cliente, documento):
        sql = """SELECT cod from cc_clientes WHERE cliente="{}" and documento="{}" """.format(nome_cliente, documento)
        self.cur.execute(sql)
        dados = self.cur.fetchall()

        if len(dados) > 0:
            self.cod_cc_cliente = dados[0][0]

        return self.cod_cc_cliente
    ...
    def enche_documentos(self, nome_documento):
        dados = enche_combobox_com_clausula(self.cur, "cc_clientes", "documento", """cliente="{}" 
    ...
    def get_valor_documento(self, documento):
        dados = enche_combobox_com_clausula(self.cur, "cc_clientes", "saldo", """documento="{}" """.format(documento))
    ...
    def closeEvent(self, evt):
        ...
        try:
            if parent is not None:
                parent.fill_table()
                parent.enchepagamento_de_clientes()
    ...
    def validacao(self):
        ...
        if self.valor_pago.value() == 0:
            ...
            return False
        elif self.cliente.currentText() == "" or self.documento.currentText() == "":
    ...
    def mostrar_registo(self, codigo):
        ...
        sql = """select c.cliente, c.documento, p.saldo_anterior, p.valor_pago, p.saldo_actual, p.obs 
        FROM pagamento_de_clientes p INNER JOIN cc_clientes c ON c.cod=p.codcc WHERE p.cod={} """.format(codigo)
    ...
    def existe(self, codigo):
        ...
        sql = """SELECT cod from pagamento_de_clientes WHERE cod = "{codigo}" """.format(codigo=str(codigo))

        self.cur.execute(sql)
        data = self.cur.fetchall()

        if len(data) == 0:
            self.cod_pagamento_de_cliente = codigo
            return False
        else:
            codigo = int(data[0][0])
            self.cod_pagamento_de_cliente = codigo
    ...
    def add_record(self):
        ...
        cod = self.cod_pagamento_de_cliente
        codcc = self.get_codcc(self.cliente.currentText(), self.documento.currentText())
        ...
        if self.existe(cod) is True:
            ...
            sql = """UPDATE pagamento_de_clientes set codcc="{codcc}", data="{data}", saldo_anterior={saldo_anterior},
            ...
            sql = """INSERT INTO pagamento_de_clientes (codcc, data, saldo_anterior, valor_pago, saldo_actual, estado, 
            obs, created, modified, modified_by, created_by) values({value})""".format(value=values)

        sql_cc_clientes = """UPDATE cc_clientes set saldo={}, valor_pago={}+valor_pago WHERE cod={} """.format(
            saldo_actual,
            valor_pago,
            self.cod_cc_cliente)

        print(sql_cc_clientes)
        print(sql)
...
if __name__ == '__main__':
    ...
    helloPythonWidget = Pagamento_de_Cliente()
