...
class Lista_de_Pagamento_de_Cliente(QMainWindow):
    def __init__(self, parent=None):
        super(Lista_de_Pagamento_de_Cliente, self).__init__(parent)

        # controla o codigo
        self.current_id = ""
        self.lista_de_pagamento_de_clientes = []
        self.list_de_dados_normal = []

        # Header for the table
        self.header = [qApp.tr('Cod'), "Data", qApp.tr('Cliente'), "Documento", qApp.tr('Saldo Anterior'),
    ...
    def fill_table(self):
        ...
        sql = """select p.cod, p.data, c.cliente, c.documento, p.saldo_anterior, p.valor_pago, p.saldo_actual, 
            p.obs, c.cod FROM pagamento_de_clientes p 
            INNER JOIN cc_clientes c ON c.cod=p.codcc WHERE  ((c.cliente like "%{nome}%" 
        ...
        try:
            ...
            self.lista_de_pagamento_de_clientes = data
    ...
    def new_data(self):
        ...
        from pagamento_de_clientes import Pagamento_de_Cliente
        cl = Pagamento_de_Cliente(self)
        cl.setModal(True)
        cl.show()
    ...
    def update_data(self):
        ...
        from pagamento_de_clientes import Pagamento_de_Cliente
        cl = Pagamento_de_Cliente(self)
        cl.cod_pagamento_de_cliente = self.current_id
        cl.mostrar_registo(self.current_id)
        cl.setModal(True)
        cl.show()

    def get_ccdata(self, codpagamento):
        sql = """SELECT codcc, valor_pago, saldo_actual from pagamento_de_clientes WHERE cod={} """.format(codpagamento)
        self.cur.execute(sql)
    ...
    def removerow(self):
        ...
        if self.current_id != "CL20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                ...
                sql = """delete from pagamento_de_clientes WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))

                sql_cc_clientes = """UPDATE cc_clientes set saldo=saldo+{}, valor_pago=valor_pago-{} WHERE cod={} 
                """.format(
                    self.valor_pago,
                    self.valor_pago,
                    self.codcc
                ...
                try:
                    self.cur.execute(sql_cc_clientes)
                    self.cur.execute(sql)
                    self.conn.commit()
                    self.fill_table()
            ...
            QMessageBox.warning(self, "Erro", "Pagamento_de_Cliente não pode ser apagado.")

    def imprime_lista(self):
        """Imprime no ecra a Lista geral de produtos baseado em um Critério"""
        if self.lista_de_pagamento_de_clientes == []:
            return

        dados = self.lista_de_pagamento_de_clientes
...
#
    helloPythonWidget = Lista_de_Pagamento_de_Cliente()
