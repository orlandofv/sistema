...
class Lista_de_Pagamento_a_Fornecedor(QMainWindow):
    def __init__(self, parent=None):
        super(Lista_de_Pagamento_a_Fornecedor, self).__init__(parent)

        # controla o codigo
        self.current_id = ""
        self.lista_de_pagamento_a_fornecedores = []
        self.list_de_dados_normal = []

        # Header for the table
        self.header = [qApp.tr('Cod'), "Data", qApp.tr('Fornecedor'), "Documento", qApp.tr('Saldo Anterior'),
                       qApp.tr('Valor Pago'), qApp.tr('Saldo Actual'), qApp.tr('Notas'), ""]
    ...
    def fill_table(self):
        ...
        sql = """select p.cod, p.data, c.fornecedor, c.documento, p.saldo_anterior, p.valor_pago, p.saldo_actual, 
            p.obs, c.cod FROM pagamento_a_fornecedores p 
            INNER JOIN cc_fornecedores c ON c.cod=p.codcc WHERE  ((c.fornecedor like "%{nome}%" 
            OR c.documento like "%{nome}%" or p.valor_pago like "%{nome}%") 
            AND p.data BETWEEN "{i}" AND "{f}") 
            """.format(nome=self.procurar_registo.text(), i=data_inicial, f=data_final)
        ...
        try:
            ...
            i = 0
            for x in data:
                x[1] = QDate.fromString(x[1], "yyyy-MM-dd").toString("dd-MM-yyyy")
                x[4] = "{:20,.2f}".format(Decimal(x[4]))
                x[5] = "{:20,.2f}".format(Decimal(x[5]))
                x[6] = "{:20,.2f}".format(Decimal(x[6]))
                i += 1

            self.lista_de_pagamento_a_fornecedores = data
    ...
    def new_data(self):
        ...
        from pagamento_a_fornecedores import Pagamento_a_Fornecedor
        cl = Pagamento_a_Fornecedor(self)
        cl.setModal(True)
        cl.show()

    def update_data(self):

        QMessageBox.warning(self, "Info", """Não pode actualizar dados do pagamento.
        \nPor favor apage e volte a inserir de novo.""")
        ...
        from pagamento_a_fornecedores import Pagamento_a_Fornecedor
        cl = Pagamento_a_Fornecedor(self)
        cl.cod_pagamento_a_fornecedor = self.current_id
        cl.mostrar_registo(self.current_id)
        cl.setModal(True)
        cl.show()

    def get_ccdata(self, codpagamento):
        sql = """SELECT codcc, valor_pago, saldo_actual from pagamento_a_fornecedores WHERE cod={} """.format(codpagamento)
        self.cur.execute(sql)
        dados = self.cur.fetchall()

        if len(dados) > 0:
            self.codcc = dados[0][0]
            self.valor_pago = dados[0][1]
            self.saldo_anterior = dados[0][2]
            return True
    ...
    def removerow(self):
        ...
        self.get_ccdata(self.current_id)
        ...
        if self.current_id != "CL20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                ...
                sql = """delete from pagamento_a_fornecedores WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))

                sql_cc_fornecedores = """UPDATE cc_fornecedores set saldo=saldo+{}, valor_pago=valor_pago-{} WHERE cod={} 
                """.format(
                    self.valor_pago,
                    self.valor_pago,
                    self.codcc
            ...
            QMessageBox.warning(self, "Erro", "Pagamento_de_Fornecedor não pode ser apagado.")

    def imprime_lista(self):
        """Imprime no ecra a Lista geral de produtos baseado em um Critério"""
        if self.lista_de_pagamento_a_fornecedores == []:
            return

        dados = self.lista_de_pagamento_a_fornecedores

        html = "<center> <h2> Lista de Trasnsacões </h2> </center>"
        html += "<hr/>"
        ...
        for fornecedor in dados:
            ...
            </tr>""".format(fornecedor[1], fornecedor[2], fornecedor[3], fornecedor[4], fornecedor[5], fornecedor[6], fornecedor[7])
        html += "</table>"
...
#
    helloPythonWidget = Lista_de_Pagamento_a_Fornecedor()
