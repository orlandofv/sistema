...
from instrutores import Instrutor

DB_FILENAME = 'dados.tsdb'


class ListaDeInstrutores(QMainWindow):
    def __init__(self, parent=None):
        super(ListaDeInstrutores, self).__init__(parent)
    ...
    def fill_table(self):
        ...
        header = [qApp.tr('Código'), qApp.tr('Nome'), qApp.tr('Apelido'), qApp.tr('Nacionalidade'),
                  qApp.tr('Email'), "Contactos", "Documento", "Comentários"]

        self.sql = """select instrutores.cod, instrutores.nome, instrutores.apelido, instrutores.nacionalidade, 
        instrutores.email, instrutores.contactos, instrutores.numero_id, instrutores.obs  
        from instrutores WHERE instrutores.nome like "%{nome}%" """.format(nome=self.find_w.text())
    ...
    def removerow(self):
        ...
        if self.current_id != "CL20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                ...
                sql = """delete from instrutores WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))
                ...
                try:
                    ...
                    QMessageBox.warning(self, "Erro", "Impossivel apagar instrutore.")
        else:
            QMessageBox.warning(self, "Erro", "Instrutore não pode ser apagado.")
    ...
    def imprime_lista(self):
        ...
        html = "<center> <h2> Lista de Instrutores </h2> </center>"
        html += "<hr/>"
...
#
#     helloPythonWidget = ListaDeInstrutores()
