...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.header = [qApp.tr('Código'), "Família",qApp.tr('Sub Família'), qApp.tr('Observações')]
    ...
    def fill_table(self):
        ...
        self.sql = """select subfamilia.cod, familia.nome, subfamilia.nome, subfamilia.obs from familia
         INNER JOIN subfamilia on familia.cod = subfamilia.codfamilia WHERE subfamilia.nome like "%{nome}%" 
    ...
    def removerow(self):
        ...
        if self.current_id != "SF20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                                    QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
                sql = """delete from subfamilia WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))
                self.cur.execute(sql)
                self.conn.commit()
                self.fill_table()
                QMessageBox.information(self, "Sucesso", "Item apagado com sucesso...")
        else:
            QMessageBox.information(self, "Erro", "Subfamília não pode ser Eliminada.")