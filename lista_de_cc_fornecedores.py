...
class Lista_de_CC_Fornecedor(QMainWindow):
    def __init__(self, parent=None):
        super(Lista_de_CC_Fornecedor, self).__init__(parent)

        # controla o codigo
        self.current_id = ""
        self.lista_de_cc_fornecedores = []
        self.list_de_dados_normal = []

        # Header for the table
        self.header = [qApp.tr('Cod'), qApp.tr('Fornecedor'), "Documento", qApp.tr('Data'), qApp.tr('IVA'),
                       qApp.tr('Valor a pagar'), qApp.tr('Valor pago'), qApp.tr('Saldo'), qApp.tr('Notas')]
    ...
    def fill_table(self):
        ...
        sql = """SELECT cod, fornecedor, documento, data, iva, valor_a_pagar, valor_pago, saldo, 
            obs FROM cc_fornecedores WHERE  ((fornecedor like "%{nome}%" 
            OR documento like "%{nome}%" or valor_a_pagar like "%{nome}%" or valor_pago like "%{nome}%") 
        ...
        try:
            ...
            i = 0
            for x in data:
                x[3] = QDate.fromString(x[3], "yyyy-MM-dd").toString("dd-MM-yyyy")
                x[4] = "{:20,.2f}".format(Decimal(x[4]))
                x[5] = "{:20,.2f}".format(Decimal(x[5]))
                x[6] = "{:20,.2f}".format(Decimal(x[6]))
                x[7] = "{:20,.2f}".format(Decimal(x[7]))
                i += 1

            print(data)
            self.lista_de_cc_fornecedores = data
    ...
    def new_data(self):
        ...
        from cc_fornecedores import CC_Fornecedor
        cl = CC_Fornecedor(self)
        cl.setModal(True)
        cl.show()
    ...
    def update_data(self):
        ...
        from cc_fornecedores import CC_Fornecedor
        cl = CC_Fornecedor(self)
        cl.cod_cc_fornecedor = self.current_id
    ...
    def removerow(self):
        ...
        if self.current_id != "CL20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                ...
                sql = """delete from cc_fornecedores WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))
            ...
            QMessageBox.warning(self, "Erro", "CC_Fornecedor não pode ser apagado.")

    def imprime_lista(self):
        """Imprime no ecra a Lista geral de produtos baseado em um Critério"""
        if self.lista_de_cc_fornecedores == []:
            return

        dados = self.lista_de_cc_fornecedores
        ...
        for fornecedor in dados:
            ...
            </tr>""".format(fornecedor[1], fornecedor[2], fornecedor[3], fornecedor[4], fornecedor[5], fornecedor[6],
                            fornecedor[7])
        html += "</table>"
...
#
    helloPythonWidget = Lista_de_CC_Fornecedor()
