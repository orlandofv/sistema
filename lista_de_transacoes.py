...
class Lista_de_Trasnsacoes(QMainWindow):
    def __init__(self, parent=None):
        super(Lista_de_Trasnsacoes, self).__init__(parent)
        ...
        self.header = [qApp.tr('Cod'), qApp.tr('Transação'), "Conta", qApp.tr('Data'), qApp.tr('Valor'), qApp.tr('Usuário'),
                  qApp.tr('Notas')]
    ...
    def fill_table(self):
        ...
        sql = """select cod, tipo_trasacao, conta, data, valor, usuario, obs 
                FROM transacoes WHERE  (tipo_trasacao like "%{nome}%" 
                OR conta like "%{nome}%" or valor like "%{nome}%" or usuario like "%{nome}%") 
        ...
        try:
            ...
            i = 0
            for x in data:
                conta = self.set_nomeconta(x[2])
                data[i][2] = conta
                data[i][3] = QDate.fromString(data[i][3], "yyyy-MM-dd").toString("dd-MM-yyyy")
                data[i][4] = "{:20,.2f}".format(Decimal(data[i][4]))
                i += 1

            self.lista_de_transacoes = data
    ...
    def update_data(self):
        ...
        from transacoes import Transacao
        cl = Transacao(self)
        cl.cod_transacao = self.current_id
    ...
    def removerow(self):
        ...
        if self.current_id != "CL20181111111":
            if QMessageBox.question(self, "Pergunta", str("Deseja eliminar o registo %s?") % self.current_id,
                ...
                sql = """delete from transacoes WHERE cod = "{codigo}" """.format(codigo=str(self.current_id))
            ...
            QMessageBox.warning(self, "Erro", "Transacao não pode ser apagado.")

    def imprime_lista(self):
        """Imprime no ecra a Lista geral de produtos baseado em um Critério"""
        if self.lista_de_transacoes == []:
            return

        dados = self.lista_de_transacoes
        ...
        html += "<th width = 25%>{}</th>".format(self.header[2])
        html += "<th width = 15%>{}</th>".format(self.header[3])
        html += "<th width = 15%>{}</th>".format(self.header[4])
        html += "<th width = 15%>{}</th>".format(self.header[5])
        html += "<th width = 20%>{}</th>".format(self.header[6])
        html += "</tr>"

        for cliente in dados:

            html += """<tr> <td>{}</td> <td>{}</td> <td>{}</td> <td>{}</td> <td>{}</td> <td>{}</td>  
            </tr>""".format(cliente[1], cliente[2], cliente[3], cliente[4], cliente[5], cliente[6])

            total = 0
            dados_n = self.list_de_dados_normal

            for x in dados_n:
                total += Decimal(x[4])

        html += """<tr style="font:bold;"> <td>{}</td> <td>{}</td> <td>{}</td> <td>{}</td> <td>{}</td> <td>{}</td>  
                    </tr>""".format("TOTAL", "", "","{:20,.2f}".format(total), "", "")
        html += "</table>"
...
#
    helloPythonWidget = Lista_de_Trasnsacoes()
